...
- 发布 CLR API 的产品项目统一启用 `CS1591` 并按错误处理；纯构建资产包 `AtomUI.City.Build` 以 machine-readable MSBuild contract 替代 CLR XML，测试、fixture 与私有构建辅助项目不属于发布 XML documentation 合同。
- Release 构建同时编译 `net10.0` 和生产基线 `net8.0`；SDK package validation 以 strict compatible-framework 模式比较两个程序集的 API。
- `engineering/check-public-api.sh` 必须真实执行全部产品模块的 Release build 与 pack：CLR 产品确认每个目标框架生成包含公开成员的 XML 文档；Build 确认 nupkg 无 `lib/` 且包含通过精确一致性测试的 MSBuild contract。不能只检查 baseline 或文件是否存在。
